...
{
    /// <summary>
    /// Dark Knight's primary attack - deals dark damage and reduces healing.
    /// </summary>
    public class DarkSlashAbility : AbilityRuntime
    {
        public override void Execute(FightContext ctx, IActor explicitTarget = null)
        {
            ...
            if (target is Game.Player.PlayerCharacter player)
            {
                ...
                // Apply a healing reduction debuff (use Slow as proxy)
                player.StatusEffects?.AddEffect(StatusEffectType.Slow, 2, 1, 1);
            }
            ...
            ctx.Log($"{Owner.DisplayName} unleashes a dark slash on {target.DisplayName} for {finalDmg} damage!");
