...
namespace Game.Ryfts
{
    ...
    public class RyftEffectManager : MonoBehaviour
    {
        ...
        private int bonusMaxHp, bonusStr, bonusDef, bonusEng, bonusIntellect;
        public int BonusMaxHp   => bonusMaxHp;
        public int BonusStrength=> bonusStr;
        public int BonusIntellect=> bonusIntellect;
        public int BonusDefense => bonusDef;
        public int BonusEngineering => bonusEng;

        // Battle-scoped temporary deltas
        private int tempMaxHp, tempStr, tempDef, tempIntellect, tempEng;
        ...
        public int TempIntellect  => tempIntellect;
        ...
        public void AddTempIntellect(int v)    { tempIntellect += v; }
        ...
        {
            tempMaxHp = tempStr = tempEng = tempIntellect = tempDef = 0;
            nextCallCostDelta = 0;
            hasLastPaid = false;
            lastPaidCost = 0;
        }
        ...
        public void PlayerPermanentStatsDelta(int maxHp = 0, int strength = 0, int eng = 0, int intellect = 0)
        {
            bonusMaxHp += maxHp;
            bonusStr   += strength;
        ...
        /// <summary>Sum intMagnitude*stacks for all active effects with matching builtIn op.</summary>
        public int SumInt(BuiltInOp op)
        {
            int total = 0;
            foreach (var r in active)
            {
                if (r?.Def != null && r.Def.builtIn == op)
                    total += r.Def.intMagnitude * r.stacks;
            }
            return total;
        }

        /// <summary>Sum floatMagnitude*stacks for all active effects with matching builtIn op.</summary>
        public float SumFloat(BuiltInOp op)
        {
            float total = 0f;
            foreach (var r in active)
            {
                if (r?.Def != null && r.Def.builtIn == op)
                    total += r.Def.floatMagnitude * r.stacks;
            }
            return total;
        }

        /// <summary>Count equipped (non-broken) equipment pieces.</summary>
        public int GetEquippedPieceCount()
        {
            ...
            foreach (Game.Equipment.EquipmentSlot slot in System.Enum.GetValues(typeof(Game.Equipment.EquipmentSlot)))
            {
                ...
                if (equipped != null && !equipped.IsBroken) count++;
            }
            return count;
        }
