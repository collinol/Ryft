...
{
    public class NecroticBlastCard : CardRuntime
    {
        public override void Execute(FightContext ctx, IActor explicitTarget = null)
        {
            ...
            int baseDmg = 2 * consumed + GetStatValue();
            int dmg = GetFinalDamage(Mathf.Max(1, baseDmg));
            bool killed = DealDamageTracked(target, dmg);

            ctx.Log($"{Owner.DisplayName} uses Necrotic Blast, consuming {consumed} Cursed for {dmg} damage.");
